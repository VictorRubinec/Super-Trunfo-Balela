...
export class EspecialModel extends CardModel {
    static id   = 'v2-especial';
    static name = 'Especial (v2)';

    static getHtmlStructure() {
        return `
            <div class="card v2-especial" id="card-preview" style="--c-base: #7B2FBE;">
    ...
    static update(cardData, previewId = 'card-preview') {
        ...
        if (cardData.foto) {
            ...
            card.style.background = 'none'; // limpa bg default se tem foto
        } else {
            if (photoBg) photoBg.style.display = 'none';
            card.style.background = ''; // restaura bg default via css
        }
    ...
    static toHTML(cardData) {
        const photoHTML = cardData.foto ? `<img class="card-photo-bg" src="${esc(cardData.foto)}" alt="" />` : '';
        ...
        return `
            <div class="card v2-especial" data-card-id="${esc(cardData.id)}" style="${colorVars(cardData.cor || '#7B2FBE')}">
                ${photoHTML}
                <div class="card-top-overlay">
